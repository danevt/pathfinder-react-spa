...
export default function CommentEdit({ comment, onClose, onSave }) {
    ...
    const editCommentHandler = async values => {
        ...
        try {
            await request(`${ENDPOINT_COMMENTS}${comment._id}`, 'PUT', data);
    ...
    const { register, formAction } = useForm(editCommentHandler, {
        text: comment.text
    });
